...
@fieldwise_init
struct term_type(Copyable, ImplicitlyCopyable, Movable):
    comptime _generic_type = SIMD[
        (DType.uint32, DType.uint64)[Int(is_macos)], 1
    ]
    comptime NCCS = 20 if is_macos else 32

    var input: Self._generic_type
    var output: Self._generic_type
    var ctrl_f: Self._generic_type
    var local_f: Self._generic_type
    var c_l: StaticTuple[c_char, Int(not is_macos)]
    var specials: StaticTuple[UInt8, Self.NCCS]
    var speed_a: Self._generic_type
    var speed_b: Self._generic_type

    comptime ECHO: Self._generic_type = 8
    comptime ICANNON: Self._generic_type = 256 if is_macos else 2

    comptime VTIME: Int = 5 if not is_macos else 17
    comptime VMIN: Int = 6 if not is_macos else 16
    ...
    fn __init__(out self):
        ...
        self.ctrl_f = 0
        self.local_f = 0
        self.c_l = type_of(self.c_l)()

        @parameter
        if type_of(self.c_l).size:
            self.c_l[0] = 0
        self.specials = type_of(self.specials)()
    ...
    fn get_attr(mut self):
        _ = external_call[
            "tcgetattr", c_int, c_int, UnsafePointer[term_type, MutAnyOrigin]
        ](0, UnsafePointer(to=self))

    fn set_attr(mut self):
        _ = external_call[
            "tcsetattr",
            c_int,
            c_int,
            c_int,
            UnsafePointer[term_type, MutAnyOrigin],
        ](0, 2, UnsafePointer(to=self))
...
struct Keys:
    comptime storage_type = SIMD[DType.uint8, 8]

    comptime backspace = Self.storage_type(127, 0, 0, 0, 0, 0, 0, 0)
    comptime tab = Self.storage_type(9, 0, 0, 0, 0, 0, 0, 0)
    comptime enter = Self.storage_type(10, 0, 0, 0, 0, 0, 0, 0)
    comptime esc = Self.storage_type(27, 0, 0, 0, 0, 0, 0, 0)
    comptime arrow_l = Self.storage_type(27, 91, 68, 0, 0, 0, 0, 0)
    comptime arrow_r = Self.storage_type(27, 91, 67, 0, 0, 0, 0, 0)
    comptime arrow_u = Self.storage_type(27, 91, 65, 0, 0, 0, 0, 0)
    comptime arrow_d = Self.storage_type(27, 91, 66, 0, 0, 0, 0, 0)
    comptime shift_arrow_l = Self.storage_type(27, 91, 49, 59, 50, 68, 0, 0)
    comptime shift_arrow_r = Self.storage_type(27, 91, 49, 59, 50, 67, 0, 0)
    comptime shift_arrow_u = Self.storage_type(27, 91, 49, 59, 50, 65, 0, 0)
    comptime shift_arrow_d = Self.storage_type(27, 91, 49, 59, 50, 66, 0, 0)

    @staticmethod
    fn is_input(arg: Self.storage_type) -> Bool:
        return (
            (
                arg.eq(0).cast[DType.uint8]().reduce_add()
                == (Self.storage_type.size - 1)
            )
            and (arg[0] >= 32)
            and (arg[0] < 127)
        )
...
@fieldwise_init
struct Events(Copyable, Movable):
    var values: QueueSIMD[DType.uint8]

    fn __init__(out self):
        self.values = type_of(self.values)()
    ...
    fn get_k(mut self, out ret: SIMD[DType.uint8, 8]):
        ...
        var ok = external_call[
            ...
            c_int,
            c_int,
            UnsafePointer[type_of(x), origin=MutAnyOrigin],
            c_int,
    ...
    try:
        ...
        while not done:
            ok = external_call[
                ...
                c_int,
                c_int,
                UnsafePointer[type_of(x), MutAnyOrigin],
                c_int,
            ](0, UnsafePointer(to=x), x.size)
            if ok > 0:
                done = True
        ...
        if len(res2) != 2:
            return 0
        return type_of(ret)(Int(res2[0]), Int(res2[1])).rotate_left[1]()
    except e:
        ...
    return 0
...
@fieldwise_init
struct TimeCounter(Copyable, Movable):
    comptime nanos_in_one_sec: UInt = 1000000000
    var previous: UInt
    ...
    fn wait_at_least(mut self, nanos: UInt = Self.nanos_in_one_sec // 60):
        ...
        if (current - self.previous) < nanos:
            var in_sec = (
                Float64(nanos) - Float64(current - self.previous)
            ) / Float64(Self.nanos_in_one_sec)
            sleep(in_sec)
            self.previous = perf_counter_ns()
        else:
...
@fieldwise_init
struct Fg(Copyable, ImplicitlyCopyable, Movable):
    comptime default = Self(39)
...
@fieldwise_init
struct Bg(Copyable, ImplicitlyCopyable, Movable):
    comptime default = Self(49)
    comptime black = Self(40)
    comptime red = Self(41)
    comptime green = Self(42)
    comptime yellow = Self(43)
    comptime blue = Self(44)
    comptime magenta = Self(45)
    comptime cyan = Self(46)
    comptime white = Self(47)
    var value: UInt8
...
):  # TODO: remove ImplicitlyCopyable ?
    var value: String
    var fg: UInt8
    var bg: UInt8

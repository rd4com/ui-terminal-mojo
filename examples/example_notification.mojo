...
def main():
    ...
    for _ in ui:
        ...
        if ui[-1].click():
            storage.append(
                Notification(message, auto_fade_second=Int(time_fade_second))
            )
        ...
        if ui[-1].click():
            storage.append(
                Notification(
                    message,
                    theme=Fg.green,
                    auto_fade_second=Int(time_fade_second),
                )
            )
        "Add notification (time extend on hover)" in ui
        if ui[-1].click():
            var tmp = Notification(
                message, theme=Fg.green, auto_fade_second=Int(time_fade_second)
            )
